...
#-----------telegram_ip_ranges------------#
if (!checktelegramip()) {
    error_log('Webhook IP rejected REMOTE_ADDR=' . ($_SERVER['REMOTE_ADDR'] ?? '')
...
$reportnight = $reportnight ?? '';
$reporttest = $reporttest ?? '';
$errorreport = $errorreport ?? '';
$porsantreport = $porsantreport ?? '';
$reportcron = $reportcron ?? '';
$reportbackup = $reportbackup ?? '';
$buyreport = $buyreport ?? '';
...
if ($text == "/start" || $datain == "start" || $text == "start") {
    ...
    if (!$nameloc) {
        sendmessage($from_id, $textbotlang['users']['stateus']['UserNotFound'] ?? '❌ سرویس یافت نشد', $keyboard, 'html');
        step('home', $from_id);
        return;
    }
    ...
    if (!is_array($marzban)) {
        sendmessage($from_id, $textbotlang['users']['stateus']['panelNotConnected'] ?? '❌ پنل سرویس در دسترس نیست', $keyboard, 'html');
        step('home', $from_id);
        return;
    }
    ...
    #--------------[ subsupdate ]---------------#
    $lastupdate = 'ثبت نشده';
    if ($DataUserOut['sub_updated_at'] !== null && $DataUserOut['sub_updated_at'] !== '') {
...
💡 برای قطع دسترسی دیگران کافیست روی گزینه \"تغییر لینک\" کلیک کنید.";
    ...
    if (!is_array($marzban_list_get)) {
        sendmessage($from_id, $textbotlang['users']['stateus']['panelNotConnected'] ?? '❌ پنل سرویس در دسترس نیست', null, 'html');
        return;
    }
...
نام کاربری کانفیک : {$nameloc['username']}";
...
} elseif (preg_match('/^configget_(.+)_(\d+)$/', $datain, $dataget)) {
    $id_invoice = $dataget[1];
    $link_index = (int) $dataget[2];
    $nameloc = select("invoice", "*", "id_invoice", $id_invoice, "select");
    if ($nameloc == false) {
        sendmessage($from_id, $textbotlang['users']['stateus']['UserNotFound'], null, 'html');
        return;
    ...
    if (!isset($DataUserOut['links'][$link_index])) {
        sendmessage($from_id, "❌ کانفیگ درخواستی یافت نشد. از منوی سرویس دوباره «دریافت کانفیگ» را بزنید.", null, 'html');
        return;
    }
    $randomString = bin2hex(random_bytes(3));
    $urlimage = "$from_id$randomString.png";
    $qrCode = createqrcode($DataUserOut['links'][$link_index]);
    if ($qrCode === null) {
        sendmessage($from_id, "❌ خطا در ساخت QR — لینک کانفیگ خالی است.", null, 'html');
        return;
    }
    file_put_contents($urlimage, $qrCode->getString());
    addBackgroundImage($urlimage, $qrCode, 'images.jpg');
    telegram('sendphoto', [
        'chat_id' => $from_id,
        'photo' => new CURLFile($urlimage),
        'caption' => "<code>{$DataUserOut['links'][$link_index]}</code>",
        'parse_mode' => "HTML",
    ]);
    unlink($urlimage);
...
⚠️ در صورتی که میخواهید مجدد کانفیگ غیرفعال شود باید از بخش مدیریت سرویس دکمه <u>❌ خاموش کردن اکانت</u>را کلیک کنید", $confirmdisableaccount);
    ...
    if (!$marzban_list_get || !is_array($marzban_list_get)) {
        sendmessage($from_id, "❌ پنل سرویس یافت نشد. نام پنل در فاکتور با تنظیمات ادمین مطابقت ندارد.", null, 'html');
        return;
    }
...
if ($user['step'] == "createusertest" || preg_match('/locationtest_(.*)/', $datain, $dataget) || ($usertestBtnPressed && $usertestSinglePanel)) {
    if (($setting['status_usertest'] ?? 'ontest') === 'offtest') {
        sendmessage($from_id, '📌 سرویس تست در حال حاضر غیرفعال است.', null, 'HTML');
        return;
    }
    if (!check_active_btn($setting['keyboardmain'], "text_usertest")) {
        sendmessage($from_id, "📌 سرویس تست در حال حاضر در دسترس نیست .", null, 'HTML');
...
🔔 حداقل حجم $mainvolume گیگابایت و حداکثر $maxvolume گیگابایت می باشد.";
    ...
    $stmt->execute();
    $countinovoice = $stmt->rowCount();
    $panelLimitRaw = trim((string) ($marzban_list_get['limit_panel'] ?? ''));
    $panelLimitInt = (int) $panelLimitRaw;
    $panelHasLimit = !in_array(mb_strtolower($panelLimitRaw), ['unlimited', 'unlimted', '0', ''], true) && $panelLimitInt > 0;
    if ($panelHasLimit) {
        if ($countinovoice >= $panelLimitInt) {
...
⚠️ حداقل زمان $maintime روز  و حداکثر $maxtime روز  می توانید تهیه کنید";
    ...
    if (!isset($dataoutput['username']) || $dataoutput['username'] === null || $dataoutput['username'] === '') {
        ...
        Editmessagetext(
            $from_id,
            $message_id,
            "❌ ساخت سرویس ناموفق بود.\n\n✍️ " . mb_substr($errorMessage, 0, 350),
            $keyboard,
            'HTML'
        );
        sendmessage($from_id, $textbotlang['users']['sell']['ErrorConfig'], $keyboard, 'HTML');
...
▫️زمان خرید : $timejalali";
    ...
    step('home', $from_id);
} elseif ($datain == "Add_Balance" || vira_is_start_param($text, 'charge')) {
    update("user", "Processing_value", "0", "id", $from_id);
    update("user", "Processing_value_one", "0", "id", $from_id);
    update("user", "Processing_value_tow", "0", "id", $from_id);
    update("user", "Processing_value_four", "0", "id", $from_id);
    ...
    $chargePrompt = "💸 مبلغ را به تومان وارد کنید:\n✅ حداقل مبلغ $minbalance — حداکثر $maxbalance تومان";
    if ($datain === "Add_Balance" && !empty($message_id)) {
        Editmessagetext($from_id, $message_id, $chargePrompt, $bakinfos, 'HTML');
    } else {
        sendmessage($from_id, $chargePrompt, $bakinfos, 'HTML');
    }
    step('getprice', $from_id);
...
if (preg_match('/Confirmpay_user_(\w+)_(\w+)/', $datain, $dataget)) {
    ...
    if ($StatusPayment['payment_status'] == "finished") {
        ...
        if (!DirectPayment($Payment_report['id_order'])) {
            return;
        }
        $Balance_id = mysqli_fetch_assoc(mysqli_query($connect, "SELECT * FROM user WHERE id = '{$Payment_report['id_user']}' LIMIT 1"));
        $priceRaw = (int) $Payment_report['price'];
        $Payment_report['price'] = number_format($priceRaw, 0);
...
روش پرداخت : درگاه ارزی ریالی اول";
        ...
        if ($pricecashback != "0") {
            $result = ($priceRaw * (int) $pricecashback) / 100;
