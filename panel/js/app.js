...
function panelT(key) {
    var defaults = {
        ...
        jsConfirmMsg: 'آیا اطمینان دارید؟ این عملیات قابل بازگشت نیست.',
        jsConfirmDefault: 'این عملیات را انجام دهید؟'
    };
    return defaults[key] || key;
}

window.showConfirm = function (msg, cb, title) {
    document.getElementById('confirm-title').textContent = title || panelT('jsConfirmTitle');
    document.getElementById('confirm-msg').textContent   = msg   || panelT('jsConfirmMsg');
    _confirmCb = cb;
...
document.querySelectorAll('[data-confirm]').forEach(function (el) {
    el.addEventListener('click', function (e) {
        e.preventDefault();
        var href = el.href;
        showConfirm(el.dataset.confirm || panelT('jsConfirmDefault'), function () {
            _lb.start();
            ...
            fetch('api/search.php?q=' + encodeURIComponent(q), { credentials: 'same-origin' })
                ...
                .then(function (data) {
                    if (!data.ok || !data.items || !data.items.length) {
                        box.innerHTML = '<a href="#">نتیجه‌ای یافت نشد</a>';
