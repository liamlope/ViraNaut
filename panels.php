...
class ManagePanel
{
    ...
    function createUser($name_panel, $code_product, $usernameC, array $Data_Config)
    {
        ...
        if ($Get_Data_Panel['type'] == "marzban") {
            ...
            $days = $expire != 0 ? max(1, (int) ceil(($expire - time()) / 86400)) : 36500;
            $gb = max(1, (int) round($data_limit / pow(1024, 3)));
            $ConnectToPanel = ilan_create_user($Get_Data_Panel, $usernameC, $gb, $days);
            if (!empty($ConnectToPanel['error'])) {
                return ['status' => 'Unsuccessful', 'msg' => $ConnectToPanel['error']];
            }
            if (!empty($ConnectToPanel['status']) && (int) $ConnectToPanel['status'] >= 400) {
                return ['status' => 'Unsuccessful', 'msg' => $ConnectToPanel['body'] ?? $ConnectToPanel['status']];
            }
            $data_Output = json_decode($ConnectToPanel['body'] ?? '', true);
            if (!is_array($data_Output) || !empty($data_Output['detail'])) {
                $Output['status'] = 'Unsuccessful';
                $Output['msg'] = is_array($data_Output) ? ($data_Output['detail'] ?? 'Ilan API error') : 'Ilan API error';
            } else {
                $sub = $data_Output['subscription_url'] ?? ($Get_Data_Panel['url_panel'] . '/sub/' . ($data_Output['username'] ?? $usernameC));
                if ($inoice != false) {
                    $sub = "https://$domainhosts/sub/" . $inoice['id_invoice'];
                }
                $Output['status'] = 'successful';
                $Output['username'] = $data_Output['username'] ?? $usernameC;
                $Output['subscription_url'] = $sub;
                $Output['configs'] = [];
            }
    ...
    function RemoveUser($name_panel, $username)
    {
        ...
        if ($Get_Data_Panel['type'] == "marzban") {
            ...
            if (isset($UsernameData['error'])) {
                $Output = array(
                    'status' => 'Unsuccessful',
                    'msg' => $UsernameData['msg'] ?? $UsernameData['error']
                );
    ...
    function extend($Method_extend, $new_limit, $time_day, $username, $code_product, $name_panel)
    {
        ...
        if ($panel['type'] == "marzban") {
            ...
            if (!in_array($extend['status'], [200, 400])) {
                return array(
                    'status' => false,
                    'msg' => $extend['msg'] ?? $extend['status']
                );
            } elseif (!empty($extend['error'])) {
                return array(
                    'status' => false,
                    'msg' => $extend['error']
                );
    ...
    function extra_volume($username_account, $code_panel, $limit_volume_new)
    {
        ...
        if ($panel['type'] == "marzban") {
            ...
            if (!in_array($volume_add['status'], [200, 400])) {
                return array(
                    'status' => false,
                    'msg' => $volume_add['msg'] ?? $volume_add['status']
                );
            } elseif (!empty($volume_add['error'])) {
                return array(
                    'status' => false,
    ...
    function extra_time($username_account, $code_panel, $limit_time_new)
    {
        ...
        if ($panel['type'] == "marzban") {
            ...
            if (!in_array($time_add['status'], [200, 400])) {
                return array(
                    'status' => false,
                    'msg' => $time_add['msg'] ?? $time_add['status']
                );
            } elseif (!empty($time_add['error'])) {
                return array(
                    'status' => false,
                    'msg' => $time_add['error']
                );
