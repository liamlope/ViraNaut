...
3️⃣ Edit Mini App URL → آدرس زیر را وارد کنید:

<code>{$mini_app_url_display}</code>

✅ پس از ذخیره، دکمهٔ مینی‌اپ در منوی ربات نمایش داده می‌شود.
HTML;
...
";
    ...
    if ($rawUrl !== $text) {
        $urlNote = "\n\n✅ آدرس اصلاح شد:";
        if (preg_match('#/panel/?$#i', rtrim($rawUrl, '/')) && !preg_match('#/panel/?$#i', $text)) {
            $urlNote .= " پسوند <code>/panel</code> حذف شد (مسیر پایه کافی است).";
        }
        if (preg_match('#:443\b#', $rawUrl) && !preg_match('#:443\b#', $text)) {
            $urlNote .= " پورت :443 حذف شد.";
        }
    }
    ...
    sendmessage($from_id, $textbotlang['Admin']['managepanel']['usernameset'] . $urlNote, $backadmin, 'HTML');
    ...
    if (isset($userdata['type']) && $userdata['type'] === 'x-ui_single') {
        sendmessage(
            $from_id,
            "🔑 <b>توکن API پنل ۳ایکس‌یوآی</b> (اختیاری)\n\nاز مسیر پنل: <b>Settings → Security → API Token</b> توکن را بسازید و اینجا بفرستید.\n\nبا توکن، اتصال پایدارتر است و مشکل CSRF/۴۰۳ کمتر می‌شود.\n\nبرای <b>رد کردن</b> و استفاده از نام‌کاربری/رمز فقط <code>-</code> را بفرستید.",
            $backadmin,
            'HTML'
        );
    ...
    if ($val === '-' || $val === 'حذف' || $val === '') {
        ...
        savedata("save", "xui_api_token", $val);
    }
    sendmessage($from_id, $textbotlang['Admin']['managepanel']['getlimitedpanel'], $backadmin, 'HTML');
    step('getlimitedpanel', $from_id);
    ...
    if (!is_array($userdata) || empty($userdata['namepanel'])) {
        sendmessage($from_id, "❌ اطلاعات مراحل قبل یافت نشد. لطفاً از اول «افزودن پنل» را تکرار کنید.", $keyboardadmin, 'HTML');
        step('home', $from_id);
        return;
    }
    ...
    if ($xui_api_token_insert === null) {
        ...
        $stmt->bindValue(':xui_api_token', $xui_api_token_insert, PDO::PARAM_STR);
    }
    try {
        $stmt->execute();
    } catch (PDOException $e) {
        error_log('getlimitedpanel INSERT: ' . $e->getMessage());
        $hint = (strpos($e->getMessage(), 'xui_api_token') !== false)
            ? "\n\nروی سرور: <code>ALTER TABLE marzban_panel ADD xui_api_token TEXT NULL;</code>\nیا: <code>php table.php</code>"
            : '';
        sendmessage($from_id, "❌ ثبت پنل ناموفق.\n<code>" . htmlspecialchars($e->getMessage(), ENT_QUOTES, 'UTF-8') . "</code>{$hint}", $keyboardadmin, 'HTML');
        step('home', $from_id);
        return;
    }
    ...
    if ($userdata['type'] == "x-ui_single" or $userdata['type'] == "alireza_single") {
        $tokenNote = !empty($userdata['xui_api_token'])
            ? "\n✅ توکن API ذخیره شد."
            : "\n💡 در صورت خطای اتصال، از منوی پنل «توکن API پنل (3x-ui)» را تنظیم کنید.";
        sendmessage($from_id, "❌ نکته :
برای فعالسازی پنل باید به منوی مدیریت پنل  رفته و گزینه های 
تنظیم شناسه اینباند و دامنه لینک ساب را حتما تنظیم نمایید در غیراینصورت کانفیگ ساخته نخواهد شد{$tokenNote}", null, 'HTML');
...
⭕️ برای مدیریت پنل یکی از گزینه های زیر را انتخاب کنید";
        ...
        if ($x_ui_check_connect['success']) {
            ...
            $text_marzban = $textbotlang['Admin']['managepanel']['errorStatusPanel'] . "علت خطا {$x_ui_check_connect['errror']}";
            sendmessage($from_id, $text_marzban, $optionalireza_single, 'HTML');
        }
...
", $option_mikrotik, 'HTML');
    ...
    $urlMsg = $textbotlang['Admin']['managepanel']['ChangedurlPanel'];
    if ($rawUrl !== $text) {
        $urlMsg .= "\n✅ آدرس نرمال‌سازی شد (پورت پیش‌فرض یا /panel اضافی حذف شد).";
    }
    outtypepanel($typepanel['type'], $urlMsg);
...
⚠️ حجم بر حسب مگابایت است.", $backadmin, 'HTML');
    ...
    step('home', $from_id);
} elseif (preg_match('/^xuihub-(ib|tst|ref|hlp|tok)-([a-f0-9]{8})$/', $datain, $xuiHub) && $adminrulecheck['rule'] == "administrator") {
    $action = $xuiHub[1];
    $hash = $xuiHub[2];
    ...
    if ($action === 'ib') {
        ...
        telegram('answerCallbackQuery', array(
            'callback_query_id' => $callback_query_id,
            'text' => $ok ? '✅ اتصال برقرار است' : '⚠️ ' . mb_substr((string) ($xuiLogin['msg'] ?? 'خطا'), 0, 120),
            'show_alert' => !$ok,
        ));
        if ($ok) {
        ...
        sendmessage(
            $from_id,
            "🔑 <b>توکن API پنل ۳ایکس‌یوآی</b>\n\nاز مسیر پنل: <b>Settings → Security → API Token</b> توکن را بفرستید.\n\nبرای <b>حذف توکن</b> و برگشت به نام‌کاربری/رمز، <code>-</code> بفرستید.",
            $backadmin,
            'HTML'
        );
        step('getxuiapitoken', $from_id);
    }
} elseif (preg_match('/^xuiib-(save|all|none)-([a-f0-9]{8})$/', $datain, $xuiBulk) && $adminrulecheck['rule'] == "administrator") {
    $cmd = $xuiBulk[1];
    $hash = $xuiBulk[2];
    $namePanel = $user['Processing_value'];
    if (!mirza_xui_admin_verify_hash($namePanel, $hash)) {
    ...
    if ($cmd === 'save') {
        ...
        if ($selected === []) {
            telegram('answerCallbackQuery', array(
                'callback_query_id' => $callback_query_id,
                'text' => 'حداقل یک اینباند انتخاب کنید.',
                'show_alert' => true,
            ));
            return;
        }
        ...
        telegram('answerCallbackQuery', array(
            'callback_query_id' => $callback_query_id,
            'text' => '✅ ذخیره شد: ' . implode(', ', $selected),
            'show_alert' => true,
        ));
        $xuiLogin = login($panel['code_panel'], true, true);
        Editmessagetext($from_id, $message_id, mirza_xui_admin_dashboard_text($panel, $xuiLogin), mirza_xui_hub_inline_keyboard($namePanel), 'HTML');
    ...
    update("marzban_panel", "datelogin", null, "name_panel", $user['Processing_value']);
    step('home', $from_id);
} elseif ($text == "🔑 توکن API پنل (3x-ui)" && $adminrulecheck['rule'] == "administrator") {
    sendmessage(
        $from_id,
        "🔑 <b>توکن API پنل ۳ایکس‌یوآی</b>\n\nاز مسیر پنل: <b>Settings → Security → API Token</b> یک توکن بسازید و اینجا بفرستید.\n\nاین روش معمولاً بدون <code>POST /login</code> و بدون مشکل CSRF/کوکی کار می‌کند و در برابر برخی ۴۰۳های WAF بهتر است.\n\nبرای <b>حذف توکن</b> و برگشت به لاگین با نام‌کاربری/رمز، فقط <code>-</code> را بفرستید.",
        $backadmin,
        'HTML'
    );
    step('getxuiapitoken', $from_id);
} elseif ($user['step'] == "getxuiapitoken") {
    ...
    $typepanel = select("marzban_panel", "*", "name_panel", $user['Processing_value'], "select");
    if ($typepanel['type'] != "x-ui_single") {
        sendmessage($from_id, "❌ این گزینه فقط برای پنل «ثنایی تک‌پورت» است.", $backadmin, 'HTML');
        step('home', $from_id);
        return;
    }
    $val = trim($text);
    if ($val === '-' || $val === 'حذف') {
        update("marzban_panel", "xui_api_token", null, "name_panel", $user['Processing_value']);
        outtypepanel($typepanel['type'], "✅ توکن API حذف شد؛ دوباره با نام‌کاربری و رمز وصل می‌شود.");
    } else {
        update("marzban_panel", "xui_api_token", $text, "name_panel", $user['Processing_value']);
        outtypepanel($typepanel['type'], "✅ توکن API ذخیره شد.\n\nاز «🎯 انتخاب اینباندها» و «🔄 تست اتصال» تنظیم را ادامه دهید.");
    }
    update("marzban_panel", "datelogin", null, "name_panel", $user['Processing_value']);
    step('home', $from_id);
...
🔗آدرس ورود : " . bot_site_https_url('panel') . "
...
6 - سفارشاتی که زمان یا حجم شان تمام شده باشد (end_of_time / end_of_volume)
";
    $Response = json_encode([
        'inline_keyboard' => [
            [
    ...
    $stmt = $pdo->prepare("SELECT * FROM invoice WHERE Status IN ('end_of_time','end_of_volume')");
    $stmt->execute();
    ...
    $stmt->execute();
    $stmt = $pdo->prepare("DELETE FROM invoice WHERE Status IN ('end_of_time','end_of_volume')");
    $stmt->execute();
    $optimizebot = "
...
✅ $countexpiredorder سرویس تمام‌شده (زمان/حجم) حذف گردید.";
...
۲ - شما می توانید آموزش بصورت فیلم ُ  متن ُ تصویر ارسال نمایید", $backadmin, 'HTML');
    ...
    if ($panel['type'] == "marzban") {
        ...
        $clientRes = get_clinets($text, $panel['name_panel']);
        if (!empty($clientRes['error'])) {
            sendmessage($from_id, $clientRes['error'], null, 'HTML');
            return;
        }
        $dec = json_decode($clientRes['body'] ?? '', true);
        $obj = is_array($dec) ? ($dec['obj'] ?? null) : null;
        if (!is_array($obj)) {
            sendmessage($from_id, $textbotlang['users']['stateus']['UserNotFound'], null, 'html');
            return;
        }
    ...
    if ($panel['type'] == "ibsng") {
        ...
        sendmessage($from_id, "✅ اینباند(های) پنل ۳x-ui از روی کاربر نمونه ذخیره شد.", $optionX_ui_single, 'HTML');
    } else {
    ...
    if ($marzban_list_get['type'] == "marzban") {
        ...
        if ($ids === []) {
            ...
            if (!empty($user_data['error'])) {
                sendmessage($from_id, $user_data['error'], null, 'HTML');
                return;
            }
            $dec = json_decode($user_data['body'] ?? '', true);
            $obj = is_array($dec) ? ($dec['obj'] ?? null) : null;
            if (!is_array($obj)) {
                sendmessage($from_id, $textbotlang['users']['stateus']['UserNotFound'], null, 'html');
                return;
