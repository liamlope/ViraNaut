...
switch ($data['actions'] ?? '') {
    ...
    case 'set_inbounds':
        ...
        if ($panel['type'] == "marzban") {
            ...
            if ($user_data == null)
                sendJsonResponse(false, "User Not Found", [], 200);
            $ids = !empty($user_data['inboundIds']) ? $user_data['inboundIds'] : array((int) ($user_data['inboundId'] ?? 0));
            $datainbound = json_encode(array_values(array_filter(array_map('intval', (array) $ids))));
