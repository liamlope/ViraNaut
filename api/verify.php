...
if ($decodedJson !== null) {
    $jsonCandidates = [
        is_string($decodedJson) ? $decodedJson : null,
        is_array($decodedJson) ? ($decodedJson['initData'] ?? null) : null,
        is_array($decodedJson) ? ($decodedJson['init_data'] ?? null) : null,
    ];
    foreach ($jsonCandidates as $value) {
        if (!is_string($value)) {
            continue;
        }
        $value = trim($value);
...
if ($rawInput !== '' && strpos($rawInput, 'hash=') !== false) {
...
$botToken = trim((string) $APIKEY);
if ($botToken === '' || strpos($botToken, '{') !== false) {
    respond_json(500, [
        'status' => false,
        'msg' => 'Bot token not configured in config.php',
        'token' => null,
    ]);
}
...
foreach ($candidates as $candidate) {
    try {
        $userData = validate_telegram_init_data($candidate, $botToken);
        break;
